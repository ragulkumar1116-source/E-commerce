    ...
    db.ref('orders').orderByChild('userId').equalTo(user.uid).once('value').then(snap => {
      ...
      container.innerHTML = `
        <div class="text-center py-5">
          ...
          <h4 class="mt-3">Error Loading Orders</h4>
        ...
        <div class="row mt-3 border-top pt-3">
          <div class="col-12">
            <div class="card bg-light border-0">
              <div class="card-body">
                <h6 class="fw-bold text-primary mb-3"><i class="bi bi-truck"></i> Shipment & Tracking Details</h6>
                <div class="row">
                  <div class="col-md-6">
                    <p class="small mb-1"><strong>Courier Service:</strong> ${order.courierName || 'N/A'}</p>
                    <p class="small mb-1"><strong>Tracking Number:</strong> ${order.trackingNumber || 'N/A'}</p>
                  </div>
                  <div class="col-md-6">
                    <p class="small mb-1"><strong>Shipment Date:</strong> ${order.shipmentDate ? formatDate(order.shipmentDate) : 'N/A'}</p>
                    <p class="small mb-1"><strong>Delivery Status:</strong> <span class="badge ${getStatusClass(order.status)}">${order.status.toUpperCase()}</span></p>
                  </div>
                </div>
                <div class="mt-3">
                  <a href="${trackingUrl}" target="_blank" class="btn btn-primary btn-sm"><i class="bi bi-geo-alt"></i> Track Parcel</a>
                </div>
              </div>
            </div>
          </div>
