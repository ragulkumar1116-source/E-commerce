...
function handleLogin(e) {
  ...
  auth.signInWithEmailAndPassword(email, pass)
    .then((result) => {
      const user = result.user;
      // Get role & restore profile if missing in database
      return db.ref(`users/${user.uid}/role`).once('value').then(roleSnap => {
        const role = roleSnap.val();
        if (!role) {
          const isEmailAdmin = (email.toLowerCase() === 'admin@epsmart.com' || user.email.toLowerCase() === 'admin@epsmart.com');
          const autoRole = isEmailAdmin ? 'admin' : 'customer';
          return db.ref(`users/${user.uid}`).set({
            name: user.displayName || (isEmailAdmin ? 'Admin' : 'Customer'),
            email: user.email,
            phone: '',
            role: autoRole,
            createdAt: firebase.database.ServerValue.TIMESTAMP
  ...
  auth.signInWithPopup(provider)
    .then((result) => {
      ...
      return userRef.once('value').then((snap) => {
        if (!snap.exists()) {
          const isEmailAdmin = (user.email.toLowerCase() === 'admin@epsmart.com');
          return userRef.set({
