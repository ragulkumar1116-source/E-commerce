    ...
    db.ref(`products/${productId}`).once('value').then(snap => {
      ...
      return cartRef.once('value').then(cartSnap => {
        ...
        return cartRef.set({
          ...
          shippingCharge: p.shippingCharge !== undefined && p.shippingCharge !== null ? p.shippingCharge : null,
  ...
  getCurrentUser().then(user => {
    ...
    Promise.all([
      ...
      Object.values(items).forEach(item => {
        ...
        const shippingVal = prod.shippingCharge !== undefined && prod.shippingCharge !== null ? prod.shippingCharge : item.shippingCharge;
