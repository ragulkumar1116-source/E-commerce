...
function getCourierTrackingUrl(courier, trackingNumber) {
  ...
  const trackingNumberClean = encodeURIComponent(trackingNumber.trim());
  const courierLower = (courier || '').toLowerCase();
  
  if (courierLower.includes('blue dart')) {
    return `https://www.bluedart.com/web/operations/track-consignment?trackId=${trackingNumberClean}`;
  } else if (courierLower.includes('delhivery')) {
    return `https://www.delhivery.com/track/package/${trackingNumberClean}`;
  } else if (courierLower.includes('dtdc')) {
    return `https://www.dtdc.in/track.html?txtid=${trackingNumberClean}`;
  } else if (courierLower.includes('st courier')) {
    return `https://stcourier.com/track/shipment?tracking_number=${trackingNumberClean}`;
  } else if (courierLower.includes('professional courier')) {
    return `http://www.tpcindia.com/multiple-tracking.aspx?connotenumber=${trackingNumberClean}`;
...
window.getCourierTrackingUrl = getCourierTrackingUrl;
